...
if uploaded_file is not None:
    ...
    st.image(np.array(Image.fromarray(
        np.array(image)).resize((700, 400), Image.LANCZOS)))
    ...
    # Show the results
    st.success(f"The plant {result['status']} with {result['prediction']} confidence.")
    
    # Display prediction probabilities
    st.subheader("Prediction Probabilities:")
    predictions_dict = {
        "Healthy": f"{predictions[0][0]*100:.2f}%",
        "Multiple Diseases": f"{predictions[0][1]*100:.2f}%", 
        "Rust": f"{predictions[0][2]*100:.2f}%",
        "Scab": f"{predictions[0][3]*100:.2f}%"
    }
